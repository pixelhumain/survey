        ...
        if( startDate && (startDate.sec > (new Date().getTime()/1000)) )
                ...
                ( Object.keys( dySObj.surveys.scenario).length > Object.keys( answers).length ) )
            {
                ...
                if(!answered)
                {
                    ...
                    $("#surveyDesc").append("<h1 class='text-center text-azure bold'> Vous avez déjà répondu à cette étapes </h1><center><a href='"+baseUrl+"/survey/co/answer/id/"+dySObj.surveys.parentSurvey.id+"/user/"+userId+"' style='' class='btn bg-azure'><span>Voir vos réponses</span></a></center>");
